...
After deployment, the feature:appSecurity[Application Security feature] provides the core capability for the Jakarta EE Security API, while the xref:reference:feature/transportSecurity-1.0.adoc[Test, Transport Security feature] enables secure TLS connections by using HTTPS.
